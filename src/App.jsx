...
async function fetchSmartSplit({ items, people }) {
  const taggedItems = items.map((i) => ({ id: i.id, name: i.name, price: i.price, tag: getItemTag(i.name) }));
...
function App() {
  ...
  return (
    <div className="app-shell">
      ...
      {shareModalOpen && (
        <ShareMessageModal bill={bill} people={people} split={split} onClose={() => setShareModalOpen(false)} />
      )}
      {qrTarget && <QrModal transaction={qrTarget} bill={bill} onClose={() => setQrTarget(null)} />}
    </div>
  );
}
...
function SettleView({ split, bill, people, onShareOpen, onQrOpen }) {
  ...
  function fillAllExact() {
    setCustomAmounts(Object.fromEntries(split.map((p) => [p.id, Number(p.total.toFixed(2))])));
  }

  function fillAllEqual() {
    const each = people.length ? grandTotal / people.length : 0;
    setCustomAmounts(Object.fromEntries(people.map((p) => [p.id, Number(each.toFixed(2))])));
  }
          ...
          <div className="settle-single-grid">
            ...
            {people.map((p) => (
              <button
                ...
                onClick={() => setSingleId((prev) => (prev === p.id ? null : p.id))}
              >
                <Avatar name={p.name} />
                  ...
                  <div className="prv2-chips">
                    ...
                    {grandTotal !== p.total && (
                      <button
                        ...
                        onClick={() => setCustomAmounts((prev) => ({ ...prev, [p.id]: Number(grandTotal.toFixed(2)) }))}
                      >
          ...
          return (
            <div key={key} className={`txn-card${done ? " txn-done" : ""}`} style={{ animationDelay: `${i * 60}ms` }}>
              ...
              <div className="txn-actions">
                <button className="icon-btn" onClick={() => onQrOpen(tx)} title="Payment QR" aria-label="Payment QR">
                  <QrCode size={14} />
                </button>
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal qr-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="qr-body">
          ...
          <small className="qr-hint">Scan or screenshot to share payment details</small>
        </div>
      </div>
    </div>
  );
